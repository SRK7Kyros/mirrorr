...
# Patterns that are only valid on Windows (drive-letter absolute)
_WIN_ABS_RE = re.compile(r"^[A-Za-z]:")

# Patterns that are only valid on Unix (root absolute)
_UNIX_ABS_RE = re.compile(r"^/")


def _validate_path(name: str, value: str) -> None:
    """Raise a clear error if a path uses the wrong format for this platform.

    Rules:
      - Relative paths (``foo``, ``./foo`` or ``.``) are always valid.
      - On Windows, absolute paths must use a drive letter (``C:/foo``).
      - On Unix, absolute paths must start with ``/``.
      - The opposite absolute format is rejected with a platform-specific fix.
    """
    # Relative paths (no leading slash, no drive letter) are always fine
    if not _IS_WINDOWS and _UNIX_ABS_RE.match(value):
        pass  # Unix absolute — continue to platform check below
    elif _IS_WINDOWS and _WIN_ABS_RE.match(value):
        pass  # Windows absolute — continue to platform check below
    elif _IS_WINDOWS and _UNIX_ABS_RE.match(value):
        raise ValueError(
            f"Invalid path for {name}: {value!r}\n"
            f"  This is a Unix-style path but you are on Windows.\n"
            f"  Use a drive letter or relative path instead:\n"
            f"    ./{value.lstrip('/')}"
            f"    C:/{value.lstrip('/')}"
        )
    elif not _IS_WINDOWS and _WIN_ABS_RE.match(value):
        raise ValueError(
            f"Invalid path for {name}: {value!r}\n"
            f"  This is a Windows-style path but you are on Unix.\n"
            f"  Use a Unix path or relative path instead:\n"
            f"    ./{value}"
            f"    /data/{value}"
        )
    else:
        # Bare name like "content" or "./content" — fine on any platform
        return
...
class MirrorrSettings(BaseModel):
    ...
    def _resolve_defaults(self) -> Self:
        ...
        # Validate user-provided paths before deriving defaults.
        path_fields: dict[str, str] = {
            "base_dir": str(self.base_dir),
            "content_dir": str(self.content_dir),
            "autoruns_dir": str(self.autoruns_dir),
            "recordings_dir": str(self.recordings_dir),
            "plugins_dir": str(self.plugins_dir),
            "engines_dir": str(self.engines_dir),
            "resolvers_dir": str(self.resolvers_dir),
            "nats_server_dir": str(self.nats_server_dir),
            "binaries_dir": str(self.binaries_dir),
            "db_file": str(self.db_file),
        }
        for name, val in path_fields.items():
            # Skip the sentinel "." (unresolved default) — only validate real values.
            if val != ".":
                _validate_path(name, val)
    ...
    def create_from_env(cls, env_file: str | Path = ".env", **overrides: Any) -> Self:
        ...
        class _EnvSettings(BaseSettings):
            ...
            def _derive_paths(self) -> Self:
                ...
                # Validate user-provided paths before deriving defaults.
                path_fields: dict[str, Path | None] = {
                    "BASE_DIR": self.BASE_DIR,
                    "CONTENT_DIR": self.CONTENT_DIR,
                    "AUTORUNS_DIR": self.AUTORUNS_DIR,
                    "RECORDINGS_DIR": self.RECORDINGS_DIR,
                    "PLUGINS_DIR": self.PLUGINS_DIR,
                    "ENGINES_DIR": self.ENGINES_DIR,
                    "RESOLVERS_DIR": self.RESOLVERS_DIR,
                    "NATS_SERVER_DIR": self.NATS_SERVER_DIR,
                    "BINARIES_DIR": self.BINARIES_DIR,
                    "DB_FILE": self.DB_FILE,
                }
                for name, val in path_fields.items():
                    if val is not None:
                        _validate_path(name, str(val))
