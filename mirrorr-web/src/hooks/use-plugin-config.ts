...
export function usePluginConfig() {
	...
	const selectedProfile = useMemo(
		() => profiles.find((p) => p.id === parseInt(profileId, 10)),
		[profileId, profiles],
	);

	const selectedEngine = useMemo(() => {
		if (!engineId) return null;
		return engines.find((e) => e.id === parseInt(engineId, 10)) ?? null;
	}, [engineId, engines]);

	const selectedResolver = useMemo(() => {
		if (!resolverId) return null;
		return resolvers.find((r) => r.id === parseInt(resolverId, 10)) ?? null;
	...
	const retryModeSchema = useMemo(() => {
		...
		if (
			!modeData?.schema?.properties ||
			Object.keys(modeData.schema.properties).length === 0
		)
			return null;
	...
	const handleProfileChange = useCallback(
		(v: string) => {
			...
			const profile = profiles.find((p) => p.id === parseInt(v, 10));
	...
	const handleRetryModeChange = useCallback(
		(mode: string) => {
			setRetryMode(mode);
			if (!selectedEngine?.retry_modes_schema) {
				setRetryConfig({});
				return;
			}
