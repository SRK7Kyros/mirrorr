...
from src.storage.models import Session, Autorun, Recording, Profile, Engine, Resolver, SessionStatus
...
protected_fields: dict[type, list[str]] = {
    Session: ["status", "recording", "started_at", "ended_at"],
}

crud_routers = APIRouter()

# ── Sessions — read-only (supervisor owns lifecycle) ─────────────────

sessions_router = APIRouter(prefix="/sessions")


@sessions_router.get("/")
async def get_all_sessions(db: AsyncSession = Depends(_get_db_session)):
    return await crud.get_all(db, Session)


@sessions_router.get("/{id}")
async def get_session(id: int, db: AsyncSession = Depends(_get_db_session)):
...
async def create_session(item: dict[str, Any] = Body(...), db: AsyncSession = Depends(_get_db_session)):
...
async def delete_session(id: int, db: AsyncSession = Depends(_get_db_session)):
    ...
    if obj.status == SessionStatus.REMUXING:
        raise HTTPException(
            status_code=409,
            detail="Session is remuxing. Wait for it to complete or fail before deleting."
        )
    if obj.status in (SessionStatus.ACTIVE, SessionStatus.RECORDING):
        try:
            ...
            if e.status_code == 504:
                # Supervisor unreachable — treat as dead, clean up the orphaned DB row
            ...
            raise
    # COMPLETED or FAILED — supervisor already gone, delete directly
    await bus.emit(MirrorrEvent.SESSION_DELETED(id=id))
    await crud.delete(db, Session, id)
...
# ── Autoruns — full CRUD (schedule + profile editable before firing) ─

autoruns_router = APIRouter(prefix="/autoruns")


@autoruns_router.get("/")
async def get_all_autoruns(db: AsyncSession = Depends(_get_db_session)):
    return await crud.get_all(db, Autorun)


@autoruns_router.get("/{id}")
async def get_autorun(id: int, db: AsyncSession = Depends(_get_db_session)):
...
async def create_autorun(item: dict[str, Any] = Body(...), db: AsyncSession = Depends(_get_db_session)):
...
async def update_autorun(id: int, item: dict[str, Any] = Body(...), db: AsyncSession = Depends(_get_db_session)):
    payload = Autorun.model_validate_json(json.dumps(item))
    try:
...
async def delete_autorun(id: int, db: AsyncSession = Depends(_get_db_session)):
...
# ── Recordings — read + delete only (snapshot, immutable) ────────────

recordings_router = APIRouter(prefix="/recordings")


@recordings_router.get("/")
async def get_all_recordings(db: AsyncSession = Depends(_get_db_session)):
    return await crud.get_all(db, Recording)


@recordings_router.get("/{id}")
async def get_recording(id: int, db: AsyncSession = Depends(_get_db_session)):
...
async def delete_recording(id: int, db: AsyncSession = Depends(_get_db_session)):
...
@profiles_router.get("/")
async def get_all_profiles(db: AsyncSession = Depends(_get_db_session)):
    return await crud.get_all(db, Profile)


@profiles_router.get("/{id}")
async def get_profile(id: int, db: AsyncSession = Depends(_get_db_session)):
...
async def create_profile(item: dict[str, Any] = Body(...), db: AsyncSession = Depends(_get_db_session)):
...
async def update_profile(id: int, item: dict[str, Any] = Body(...), db: AsyncSession = Depends(_get_db_session)):
    payload = Profile.model_validate_json(json.dumps(item))
    try:
...
async def delete_profile(id: int, db: AsyncSession = Depends(_get_db_session)):
...
async def _send_control(session_id: int, command: str, timeout: float = 5.0) -> dict:
    """Send a control command to a session supervisor via NATS request/reply.

    Returns the supervisor's reply. Raises HTTPException if the supervisor
    is unreachable (timeout) or responds with an error.
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=504,
            detail=f"Session supervisor {session_id} is not responding",
        )
