...
export function DateTimePicker({
	...
	const [seconds, setSeconds] = useState(
		parsed ? String(parsed.getSeconds()).padStart(2, "0") : "",
	);
	...
	const emitValue = useCallback(
		(day: number | null, h: string, m: string, s: string) => {
			if (day === null) return;
	...
	const handleHoursChange = useCallback(
		(v: string) => {
			setHours(v);
			if (selectedDay !== null) emitValue(selectedDay, v, minutes, seconds);
		},
		[emitValue, minutes, seconds, selectedDay],
	);
	const handleMinutesChange = useCallback(
		(v: string) => {
			setMinutes(v);
			if (selectedDay !== null) emitValue(selectedDay, hours, v, seconds);
		},
		[emitValue, hours, seconds, selectedDay],
	);
	const handleSecondsChange = useCallback(
		(v: string) => {
			setSeconds(v);
			if (selectedDay !== null) emitValue(selectedDay, hours, minutes, v);
		},
		[emitValue, hours, minutes, selectedDay],
	);
				...
				<div className="p-3 w-[284px]">
					<div className="flex items-center gap-1.5 mb-3">
						...
						>
							...
							<SelectContent>
								{Array.from({ length: 21 }, (_, i) => currentYear - 5 + i).map(
									(y) => (
						...
						{calendarDays.map((cell) => {
							...
							return (
								<button
									...
									type="button"
									className={cn(
										"h-9 w-9 rounded-xl text-sm flex items-center justify-center transition-colors",
					...
					<div className="mt-3 pt-3 border-t flex items-center justify-center">
						<MaskedSlot
							...
							placeholder="HH"
						/>
						<span className="text-sm font-mono text-muted-foreground select-none mx-0.5">
							:
						</span>
						<MaskedSlot
							ref={minutesRef}
