    ...
    async function handleImport() {
        ...
        try {
            ...
            if (result.profiles_skipped)
                msgs.push(
                    `${result.profiles_skipped} profile(s) skipped (identical)`,
                );
...
function ProfilePreviewCard({
    ...
    return (
        <Collapsible open={open} onOpenChange={setOpen}>
            <div
                className={cn("rounded-lg border transition-colors", sc.color)}
            >
        ...
        <div className="space-y-1">
            ...
            <div className="rounded border bg-muted/20 overflow-hidden divide-y divide-border/50">
                {entries.map(([key, value]) => (
                    ...
                    >
                        <code className="font-mono font-semibold text-foreground text-[11px] shrink-0">
                            {key}
                        </code>
                        <ConfigValue
                            value={
...
function ConfigValue({ value }: { value: string }) {
    ...
    if (!needsTruncate) {
        return (
            <code className="text-[11px] font-mono text-muted-foreground text-right flex-1 min-w-0">
                {value}
            </code>
        );
    }
        ...
        return (
            <div className="flex items-center gap-2 rounded border border-blue-500/30 bg-blue-500/5 px-3 py-2 text-xs text-blue-700 dark:text-blue-300">
                ...
                <span>
                    ...
                    <strong>
                        {issue.new_name ?? `${issue.bundled?.name}_2`}
                    </strong>
                </span>
            </div>
        );
    ...
    if (issue.problem === "profile_missing") {
        return (
            <div className="flex items-center gap-2 rounded border border-red-500/30 bg-red-500/5 px-3 py-2 text-xs text-red-700 dark:text-red-300">
                <AlertTriangle className="size-3.5 shrink-0" />
                <span>
                    References profile <strong>{issue.profile_name}</strong>{" "}
        ...
        <div className="rounded border px-3 py-2.5 space-y-2">
            ...
            {issue.alternatives && issue.alternatives.length > 0 && (
                <div className="space-y-1">
                    ...
                    <Select
                        value={
                            currentSelection ? String(currentSelection.id) : ""
                        }
                        onValueChange={(v) =>
                            onChange(
                                currentHash,
