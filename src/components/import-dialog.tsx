...
import { ChevronRight, Package, CalendarClock, Cpu, Zap, CheckCircle2, AlertTriangle, Info, Copy, Check } from "lucide-react"
...
export function ImportDialog({
  ...
  const [pluginMap, setPluginMap] = useState<Record<string, { type: string; id: number }>>({})
  ...
  useEffect(() => {
    ...
    setPhase("loading")
    setPluginMap({})
    importExportApi.validate(bundle)
      .then((data) => {
        ...
        for (const p of r.profiles) {
          for (const issue of p.issues) {
            if (issue.bundled && issue.alternatives?.length) {
              const best = issue.alternatives.find((a) => a.name === issue.bundled!.name) ?? issue.alternatives[0]
              initialMap[issue.bundled.origin_hash] = { type: issue.field ?? "", id: best.id }
            }
          }
        }
        for (const a of r.autoruns) {
          for (const issue of a.issues) {
            if (issue.bundled && issue.alternatives?.length) {
              const best = issue.alternatives.find((al) => al.name === issue.bundled!.name) ?? issue.alternatives[0]
              initialMap[issue.bundled.origin_hash] = { type: issue.field ?? "", id: best.id }
            }
          }
        }
  ...
  function allResolved(): boolean {
    ...
    for (const p of report.profiles) {
      for (const issue of p.issues) {
        if (issue.problem === "name_conflict" || issue.problem === "already_exists") continue
  ...
  async function handleImport() {
    ...
    try {
      ...
      const msgs: string[] = []
      if (result.profiles_created) msgs.push(`${result.profiles_created} profile(s) created`)
      if (result.autoruns_created) msgs.push(`${result.autoruns_created} autorun(s) created`)
      if (result.profiles_skipped) msgs.push(`${result.profiles_skipped} profile(s) skipped (identical)`)
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col">
            ...
            {(phase === "confirm" || phase === "resolve") && (
              <>
                {profiles.map((p, i) => {
                  ...
                  const status = issues.length === 0 ? "ok" as const
                    : issues.some(i => i.problem === "already_exists") ? "skip" as const
                    : issues.some(i => i.problem === "name_conflict") ? "rename" as const
                    : "issue" as const
                  return <ProfilePreviewCard key={i} profile={p} status={status} issues={issues} pluginMap={pluginMap} onPluginChange={setPluginMapping} />
                })}
                {autoruns.map((a, i) => {
                  ...
                  return <AutorunPreviewCard key={i} autorun={a} status={status} issues={issues} pluginMap={pluginMap} onPluginChange={setPluginMapping} />
                })}
              </>
            )}
          </div>
        ...
        <DialogFooter>
          <Button variant="ghost" size="sm" className="h-7 text-[11px]" onClick={onClose}>Cancel</Button>
          {(phase === "confirm" || phase === "resolve") && (
            <Button size="sm" className="h-7 text-[11px]" onClick={handleImport} disabled={importing || (phase === "resolve" && !allResolved())}>
...
function ProfilePreviewCard({ profile, status, issues, pluginMap, onPluginChange }: {
  ...
  const resolverConfig = profile.resolver_config as Record<string, unknown> | undefined
  const retryMode = (profile.retry_mode as string) ?? "none"
  const retryConfig = profile.retry_config as Record<string, unknown> | undefined
  const contentHash = profile.content_hash as string | undefined

  const sc = {
    ok: { color: "border-emerald-500/30", bg: "bg-emerald-500/5", Icon: CheckCircle2, ic: "text-emerald-500", label: "Ready to import" },
    skip: { color: "border-emerald-500/30", bg: "bg-emerald-500/5", Icon: Info, ic: "text-emerald-500", label: "Already exists — will be skipped" },
    rename: { color: "border-blue-500/30", bg: "bg-blue-500/5", Icon: AlertTriangle, ic: "text-blue-500", label: `Will be renamed to ${issues.find(i => i.new_name)?.new_name ?? `${name}_2`}` },
    issue: { color: "border-amber-500/30", bg: "bg-amber-500/5", Icon: AlertTriangle, ic: "text-amber-500", label: "Needs resolution" },
  }[status]
      ...
      <div className={cn("rounded-lg border transition-colors", sc.color)}>
        <CollapsibleTrigger className="w-full">
          <div className={cn("px-4 py-3", sc.bg)}>
            <div className="flex items-center gap-3">
              <ChevronRight className={cn("size-4 shrink-0 transition-transform text-muted-foreground", open && "rotate-90")} />
            ...
            <div className="flex items-center gap-4 ml-7 mt-1.5">
              ...
              {retryMode !== "none" && (
                <span className="text-[11px] text-muted-foreground">Retry: {retryMode}</span>
              )}
            </div>
          </div>
        </CollapsibleTrigger>
        <CollapsibleContent>
          <div className="px-4 pb-4 pt-2 space-y-3 border-t">
            ...
            {/* Resolver config */}
            {resolverConfig && Object.keys(resolverConfig).length > 0 && <ConfigPreview title="Resolver Config" config={resolverConfig} />}

            {/* Retry config — includes mode as a row */}
            {retryMode !== "none" && retryConfig && Object.keys(retryConfig).length > 0 && (
              <ConfigPreview title={`Retry Config (${retryMode})`} config={retryConfig} />
            )}

            {/* Content hash */}
            {contentHash && <p className="text-[10px] text-muted-foreground font-mono">Hash: {contentHash}</p>}

            {/* Issues */}
            {issues.map((issue, i) => <IssueRow key={i} issue={issue} pluginMap={pluginMap} onChange={onPluginChange} />)}
          </div>
        </CollapsibleContent>
      </div>
    </Collapsible>
...
function AutorunPreviewCard({ autorun, status, issues, pluginMap, onPluginChange }: {
  ...
  const sc = status === "ok"
    ? { color: "border-emerald-500/30", bg: "bg-emerald-500/5", Icon: CheckCircle2, ic: "text-emerald-500", label: "Ready to import" }
    : { color: "border-amber-500/30", bg: "bg-amber-500/5", Icon: AlertTriangle, ic: "text-amber-500", label: "Needs resolution" }
          ...
          <div className={cn("flex items-center gap-3 px-4 py-3", sc.bg)}>
            ...
            <div className="flex-1 min-w-0 text-left">
              ...
              <p className="text-[11px] text-muted-foreground truncate">
                Profile: {profileName ?? "?"} · {recording ? "Recording" : "Stream only"}
              </p>
            </div>
            <div className="flex items-center gap-1.5 shrink-0">
              <sc.Icon className={cn("size-3.5", sc.ic)} />
          ...
          <div className="px-4 pb-4 pt-1 space-y-3 border-t">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <p className="text-xs">{startTime ? new Date(startTime).toLocaleString(undefined, { hour12: false }) : "—"}</p>
              </div>
              <div className="space-y-1">
                <Label className="text-[10px] text-muted-foreground uppercase tracking-wider">End</Label>
                <p className="text-xs">{endTime ? new Date(endTime).toLocaleString(undefined, { hour12: false }) : "—"}</p>
              </div>
            </div>
            {engineOverride && (
              <PluginCell icon={Cpu} label="Engine Override" plugin={engineOverride} />
            )}
            {contentHash && <p className="text-[10px] text-muted-foreground font-mono">Hash: {contentHash}</p>}
            {issues.map((issue, i) => <IssueRow key={i} issue={issue} pluginMap={pluginMap} onChange={onPluginChange} />)}
          </div>
...
function PluginCell({ icon: Icon, label, plugin }: {
        ...
        <div className="flex items-center gap-1">
          ...
          <p className={cn(
            ...
            {copied ? (
              ...
              <><span className="truncate">{plugin?.origin_hash?.slice(0, 16) + "…" ?? "?"}</span><Copy className="size-3 shrink-0 opacity-50" /></>
            )}
          </p>
        </div>
      </div>
...
function ConfigPreview({ title, config }: { title: string; config: Record<string, unknown> }) {
      ...
      <div className="rounded border bg-muted/20 overflow-hidden divide-y divide-border/50">
        {entries.map(([key, value]) => (
          <div key={key} className="flex items-center gap-3 py-1.5 px-3">
            <code className="font-mono font-semibold text-foreground text-[11px] shrink-0">{key}</code>
            <ConfigValue value={typeof value === "string" ? value : JSON.stringify(value)} />
          </div>
        ))}
      </div>
    </div>
...
function ConfigValue({ value }: { value: string }) {
  ...
  return (
    ...
    >
      <code className={cn(
        "text-[11px] font-mono text-muted-foreground truncate min-w-0 text-right transition-all duration-200",
      ...
      <span className={cn(
        ...
        {copied ? <Check className="size-3 text-emerald-500" /> : <Copy className="size-3 text-muted-foreground opacity-50" />}
      </span>
    </div>
  )
}
...
function IssueRow({ issue, pluginMap, onChange }: {
  issue: Issue
  pluginMap: Record<string, { type: string; id: number }>
  ...
  if (issue.problem === "name_conflict") {
    return (
      <div className="flex items-center gap-2 rounded border border-blue-500/30 bg-blue-500/5 px-3 py-2 text-xs text-blue-700 dark:text-blue-300">
        ...
        <span>Name conflict — will be imported as: <strong>{issue.new_name ?? `${issue.bundled?.name}_2`}</strong></span>
      </div>
    )
  }
  if (issue.problem === "profile_missing") {
    return (
      <div className="flex items-center gap-2 rounded border border-red-500/30 bg-red-500/5 px-3 py-2 text-xs text-red-700 dark:text-red-300">
        <AlertTriangle className="size-3.5 shrink-0" />
        <span>References profile <strong>{issue.profile_name}</strong> which is not available.</span>
      </div>
    )
  }
  ...
  return (
    <div className="rounded border px-3 py-2.5 space-y-2">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-muted-foreground font-mono truncate max-w-[120px]">{currentHash}</span>
      </div>
      <p className="text-[11px] text-muted-foreground">
        {issue.problem === "not_installed" ? "Not installed" : "Hash mismatch — plugin was updated"}
      </p>
      {issue.alternatives && issue.alternatives.length > 0 && (
        <div className="space-y-1">
          <Label className="text-[10px]">Map to:</Label>
          <Select value={currentSelection ? String(currentSelection.id) : ""} onValueChange={(v) => onChange(currentHash, issue.field ?? "", parseInt(v))}>
            <SelectTrigger className="h-7 text-xs"><SelectValue placeholder="Select plugin" /></SelectTrigger>
            <SelectContent>
              {issue.alternatives.map((alt) => (
                <SelectItem key={alt.id} value={String(alt.id)}>
                  {alt.name} <span className="text-[10px] text-muted-foreground ml-1">{alt.origin_hash.slice(0, 8)}…</span>
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
      ...
      {issue.alternatives && issue.alternatives.length === 0 && (
        <p className="text-[11px] text-destructive">No plugins available to resolve this.</p>
      )}
    </div>
  )
}
