...
export const MaskedSlot = React.forwardRef<HTMLInputElement, MaskedSlotProps>(
  (
    { digits, max, value, onChange, onBackspaceEmpty, onFull, isActive, onFocus, placeholder, disabled, className },
    ref,
  ) => {
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (value.length === digits && /\d/.test(e.key)) { e.preventDefault(); onFull?.(); return }
    ...
    return (
      <input
        ...
        style={{ width: `${digits * 2}ch`, flexShrink: 0, boxSizing: "content-box" }}
      />
    )
  },
)
  ...
  while (i < format.length) {
    ...
    if (/[A-Za-z]/.test(ch)) {
      ...
      parts.push({
        ...
        name,
        digits: name.length,
        max: TOKEN_MAX[name] ?? Math.pow(10, name.length) - 1,
        label: UNIT_LABELS[name] ?? name,
      })
      i = j
    } else {
      parts.push({ kind: "sep", char: ch, display: ch === "T" ? "-" : ch })
      i++
    }
  }
  return parts
...
export function TimeInput({
  ...
  const slots = React.useMemo(() => parts.filter((p): p is TimeSlotDef => p.kind === "slot"), [parts])
  const slotRefs = React.useRef<(HTMLInputElement | null)[]>([])

  const [activeIdx, setActiveIdx] = React.useState<number | null>(null)

  // Slot values: one string per slot, e.g. ["01", "06", "21", "21", "18", "19"]
  const [slotValues, setSlotValues] = React.useState<string[]>(() => slots.map(() => ""))
  ...
  React.useEffect(() => {
    const ms = slotValues.reduce((acc, val, i) => {
      ...
      switch (unit) {
        case "Years": return acc + n * 365.25 * 86400000
        case "Months": return acc + n * 30.44 * 86400000
        case "Days": return acc + n * 86400000
        case "Hours": return acc + n * 3600000
        case "Minutes": return acc + n * 60000
        case "Seconds": return acc + n * 1000
        default: return acc
      }
    }, 0)
    onChange?.(ms)
  ...
  return (
    <div
      ...
      style={{ gridTemplateColumns: `repeat(${parts.length}, min-content)`, gridTemplateRows: "auto auto" }}
    >
      {parts.map((part, i) => {
        if (part.kind === "sep") {
          return (
