 ...
 *       <span className="text-muted-foreground/60 text-2xs uppercase tracking-wider">Label</span>
 *       ...
 *     </span>
...
export function MetadataBar({ children, className }: MetadataBarProps) {
  return (
    <div className={cn("rounded-lg border bg-muted/20 px-3 py-2 flex flex-wrap items-center gap-x-5 gap-y-1 text-xs text-muted-foreground", className)}>
      {children}
    </div>
  )
}
...
export function MetadataItem({ label, children, className }: MetadataItemProps) {
  return (
    <span className={cn("inline-flex items-center gap-1.5", className)}>
      <span className="text-muted-subtle text-2xs uppercase tracking-wider">{label}</span>
      {children}
    </span>
  )
}
