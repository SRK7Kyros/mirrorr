                ...
                <DetailHeader
                    ...
                    actions={
                        <div className="flex items-center gap-1">
                            ...
                            <ExportButton
                                onExport={() =>
                                    importExportApi.exportAutorun(autorun.id)
                                }
                ...
                fields={[
                    {
                        ...
                        value: (
                            <StatusBadge
                                status={autorun.status ?? "scheduled"}
                            />
                        ),
                    },
                    ...
                    },
                    {
                        label: "Resolver",
                        value:
                            resolver?.name ?? `#${autorun.resolver_id ?? "?"}`,
                    },
                    {
                        label: "Retry Mode",
                        value: autorun.retry_mode ?? "none",
                    },
                ]}
            />
            <div className="flex gap-4">
                {autorun.start_time && (
                    <div className="space-y-1 shrink-0">
                        ...
                        <p className="text-xs">
                            {formatLocalDate(autorun.start_time)}
                        </p>
                    </div>
                )}
                {autorun.end_time && (
                    <div className="space-y-1 shrink-0">
                        ...
                        <p className="text-xs">
                            {formatLocalDate(autorun.end_time)}
                        </p>
                    </div>
                )}
            </div>
...
function CreateAutorunPanel({ onClose }: { onClose: () => void }) {
    ...
    const createMutation = useMutation({
        ...
        onError: (err: Error) =>
            toast.error(`Failed to create autorun: ${err.message}`),
    });
            ...
            <div className="flex flex-col gap-3">
                ...
                >
                    ...
                    <TabsContent value="relative" className="space-y-3 mt-3">
                        <div className="grid grid-cols-2 gap-3">
                            <FormField label="Start (relative)">
                                <RelativeTimeInput
                                    format="MM-DD-HH:mm:ss"
                                    value={null}
                                    onChange={setRelativeStartOffset}
                                />
                            ...
                            <FormField label="End (relative)">
                                <RelativeTimeInput
                                    format="MM-DD-HH:mm:ss"
                                    value={null}
                                    onChange={setRelativeEndOffset}
                                />
                                {relativeEndOffset > 0 && (
                                    <EtaDisplay
                                        value={new Date(
