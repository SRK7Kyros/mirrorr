		...
		>
			<InfoGrid
				fields={[
					{
						label: "Status",
						value: <StatusBadge status={autorun.status ?? "scheduled"} />,
					},
			...
			<div className="flex gap-4">
				{autorun.start_time && (
					<div className="space-y-1 shrink-0">
						...
						<p className="text-xs">{formatLocalDate(autorun.start_time)}</p>
					</div>
				)}
				{autorun.end_time && (
					<div className="space-y-1 shrink-0">
						<Label className="text-xs text-muted-foreground">End Time</Label>
						<p className="text-xs">{formatLocalDate(autorun.end_time)}</p>
					</div>
				)}
			</div>
		</DetailLayout>
